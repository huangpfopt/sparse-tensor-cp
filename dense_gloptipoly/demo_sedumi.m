...
save_path = sprintf(".\\results\\glopti_%s_%d.txt",T_name(1:end-4),lvt);
%save_path = sprintf(".\\results\\glopti_noncp%s_%d.txt",T_name(1:end-4),lvt);
diary(save_path);
diary on;

%% gloptipoly model
[output,sedumi_info] = glopti_model(T,n,d,lvt,F); %sedumi_info.feasration < 0 indicates that infeasible
fprintf("sedumi_info:\n")
disp(sedumi_info)
